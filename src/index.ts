  ...
  table(name: string) {
    const functions = {
      get: (conditions: Conditions_Type) => {
        return new Promise((resolve, reject) =>
          this.query(
            SQLConstructor.SELECT(
              "*",
              name,
              SQLConstructor.parseConditions(conditions)
            )
          )
            .then(resolve)
            .catch(reject)
      ...
      insert: (data: InsertData) => {
        return new Promise((resolve, reject) => {
          ...
          keys.forEach((key: string) => args.push(data[key]));
          this.query(SQLConstructor.INSERT(name, keys), args)
            .then(resolve)
            .catch(reject);
        });
      },
      update: (data: InsertData, conditions: Conditions_Type) => {
        return new Promise((resolve, reject) => {
          var args: Array<any> = [];
          var keys: Array<string> = Object.keys(data);
          keys.forEach((key: string) => args.push(data[key]));
          this.query(
            SQLConstructor.UPDATE(
              name,
              keys,
              SQLConstructor.parseConditions(conditions)
            ),
      ...
      delete: (conditions: Conditions_Type) => {
        return new Promise((resolve, reject) => {
          this.query(
            SQLConstructor.DELETE(
              name,
              SQLConstructor.parseConditions(conditions)
            )
          )
